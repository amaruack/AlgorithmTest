...
public class Test01 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(test.plus("00:59:24", "99:10:50"));

        System.out.println(test.compareTime(test.plus("00:59:24", "99:10:50"), "49:24:24"));
        System.out.println(test.compareTime(test.plus("00:59:24", "99:10:50"), "049:24:24"));
    ...
    public String solution(String play_time, String adv_time, String[] logs) {
        ...
        if ( compareTime(play_time, adv_time) <= 0 ) {
            return answer;
        }

        List<String[]> logsList = Arrays.stream(logs).map(log -> log.split("-")).sorted().collect(Collectors.toList());


        for (int i = 0; i < logsList.size() ; i++) {

            String startTimeString = logsList.get(i)[0];
            String endTimeString = logsList.get(i)[1];
            String plusAdv = plus(startTimeString, endTimeString);

            String finishTime = compareTime(endTimeString, plusAdv) > 0 ? endTimeString : plusAdv;
            int count = 0;
            for (int j = i; j < logsList.size() ; j++) {

                String innerStartTimeString = logsList.get(j)[0];
                String innerEndTimeString = logsList.get(j)[1];

//                if (compareTime(startTimeString, innerStartTimeString) <= 0
//                    &&
//                compareTime(startTimeString, innerStartTimeString) <= 0
//                ) {
//
//                }
    ...
    public final int HOUR_INDEX = 0;
    public final int MINUTE_INDEX = 1;
    public final int SECOND_INDEX = 2;

    public final String SPLIT_TIME = ":";

    public int compareTime(String timeStr1, String timeStr2){
        int result = 0;

        String[] timeList1 = timeStr1.split(SPLIT_TIME);
        String[] timeList2 = timeStr2.split(SPLIT_TIME);

        result = Integer.compare(Integer.parseInt(timeList1[HOUR_INDEX]),Integer.parseInt(timeList2[HOUR_INDEX]));
        if (result == 0) {
            result = Integer.compare(Integer.parseInt(timeList1[MINUTE_INDEX]),Integer.parseInt(timeList2[MINUTE_INDEX]));
            if(result == 0) {
                result = Integer.compare(Integer.parseInt(timeList1[SECOND_INDEX]),Integer.parseInt(timeList2[SECOND_INDEX]));
            }
        }
    ...
    public String plus(String timeStr1, String timeStr2) {
        String[] timeList1 = timeStr1.split(SPLIT_TIME);
        String[] timeList2 = timeStr2.split(SPLIT_TIME);

        int sumSecond = Integer.parseInt(timeList1[SECOND_INDEX]) + Integer.parseInt(timeList2[SECOND_INDEX]);

        int minup = sumSecond / 60;
        int secondRemain = sumSecond % 60;

        int sumMinute = Integer.parseInt(timeList1[MINUTE_INDEX]) + Integer.parseInt(timeList2[MINUTE_INDEX]) + minup;

        int hourup = sumMinute / 60;
        int minuteRemain = sumMinute % 60;

        int sumHour = Integer.parseInt(timeList1[HOUR_INDEX]) + Integer.parseInt(timeList2[HOUR_INDEX]) + hourup;

        return new StringBuilder().append(sumHour)
                .append(SPLIT_TIME)
                .append(minuteRemain)
                .append(SPLIT_TIME)
                .append(secondRemain)
